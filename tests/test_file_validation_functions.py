...
class TestFileValidationFunctions:
    ...
    @patch("magic.Magic")
    def test_is_valid_image_valid(self, mock_magic, mock_image_open, valid_image_data):
        # Arrange:
    ...
    def test_is_valid_image_corrupt(
        self, mock_magic, mock_image_open, invalid_image_data
    ):
        # Arrange:
        mock_mime = mock_magic.return_value
    ...
    def test_is_valid_file_valid(self, mock_magic, magic_docx):
        ...
        mock_file = FileStorage(stream=BytesIO(file_data), filename="example1.docx")
    ...
    def test_is_valid_file_corrupt_docx(
        self, mock_magic, magic_docx, invalid_image_data
    ):
        # Arrange:
        mock_mime = mock_magic.return_value
        mock_mime.from_buffer.return_value = (
        ...
        magic_docx.side_effect = docx.opc.exceptions.PackageNotFoundError(
            "Invalid format"
        )
