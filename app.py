        ...
        with Image.open(io.BytesIO(file_content)) as img:
            ...
            if img.format == 'JPEG':
                ...
                while offset < len(file_content):
                    chunk_length, chunk_type = struct.unpack('>I4s', file_content[offset:offset+8])
                    if chunk_type in [b'IEND', b'IHDR']:
                        break
                    if chunk_type not in [b'IHDR', b'IDAT', b'IEND', b'PLTE', b'tRNS', b'cHRM', b'gAMA', b'iCCP', b'sBIT', b'sRGB', b'tEXt', b'zTXt', b'iTXt', b'bKGD', b'hIST', b'pHYs', b'sPLT', b'tIME']:
    ...
    try:
        ...
        for page in reader.pages:
            if "/JS" in page or "/JavaScript" in page:
                logger.warning("PDF contains JavaScript, which could be potentially harmful.")
                return False

        # Check for embedded files
        if "/EmbeddedFiles" in reader.trailer["/Root"]:
            logger.warning("PDF contains Embedded Files, which could potentially harmful.")
            return False
        ...
        if any(keyword in pdf_text for keyword in suspicious_keywords):
            logger.warning(f"PDF contains suspicious keywords: {[kw for kw in suspicious_keywords if kw in pdf_text]}")
            return False
        ...
        if url_pattern.search(pdf_text):
            logger.warning(f"PDF contains external links, which could lead to potentially harmful content.\nURL: {url_pattern.search(pdf_text)}")
            return False
...
def is_valid_docx(uploaded_file):
    ...
    try: 
        ...
        if "word/vbaProject.bin" in zip_file.namelist():
            logger.warning("DOCX file contains macros, which could be potentially harmful")
            return False

        # Check for external links
        for rel in doc.part.rels.values():
            if rel.reltype == docx.opc.constants.RELATIONSHIP_TYPE.HYPERLINK:
                logger.warning("DOCX file contains external links, which could be potentially harmful")
                return False

        # Check for embedded objects
        if any("word/embeddings" in name for name in zip_file.namelist()):
            logger.warning("DOCX file contains embedded objects, which could be potentially harmful")
            return False
        ...
        if any(keyword in full_text.lower() for keyword in suspicious_keywords):
            logger.warning(f"DOCX file contains suspicious keywords: {[kw for kw in suspicious_keywords if kw in full_text.lower()]}")
            return False
    ...
    try:      
        ...
        with tempfile.NamedTemporaryFile(delete=True) as temp_file:
            ...
            if ole.exists('\\1Table'):
                ...
                if b'HTTP://' in table_data.upper() or b'HTTPS://' in table_data.upper():
                    logger.warning("Document contains external links. Potential security risk.")
                    ole.close()
                    return False

            # Check for embedded objects
            if ole.exists('ObjectPool'):
                logger.warning("Document contains embedded objects. Potential security risk.")
                ole.close()
                return False
